...
//****************************************************************************************************************
#define NODEID        10 //the ID of this node
#define NETWORKID     100 //the network ID of all nodes this node listens/talks to
#define FREQUENCY     RF69_915MHZ //Match this with the version of your Moteino! (others: RF69_433MHZ, RF69_868MHZ)
#define ENCRYPTKEY    "" //identical 16 characters/bytes on all nodes, not more not less!
...
void loop()
{
  ...
  {
    inputLen = readSerialLine(input, 10, 65, 100);
  ...
  {
    byte buffLen = radio.DATALEN;
    int rssi = radio.RSSI;
    byte nodeId = radio.SENDERID;
    
    if (radio.DATALEN > 0)
    {
      memcpy(&buff, (byte*) radio.DATA, min(sizeof(buff), radio.DATALEN));
    }
