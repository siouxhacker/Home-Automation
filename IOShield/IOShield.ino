...
void setup(void)
{
  ...
  radio.sendWithRetry(GATEWAYID, "START", 5, 3, 75);
  zonesOFF();
  Blink(LED, 100);
  Blink(LED, 100);
  Blink(LED, 100);
...
void zoneON(byte which)
{
  ...
  if (which > 4) which--;
  sprintf(buff, "ZONE:%d X:%d", which, radio._transmitLevel);
  if (radio.sendWithRetry(GATEWAYID, buff, strlen(buff)), 3, 75)
    {DEBUGln(F("..OK"));}
  else {DEBUGln(F("..NOK"));}
}
...
void zonesOFF()
{
  ...
  sprintf(buff, "ZONES:OFF X:%d", radio._transmitLevel);
